...
//   // ✅ Ask user on mobile if they want to use camera or upload
//   const handleUploadClick = () => {
//     // if (isMobile) {
//     //   const useCamera = confirm(
//     //     "Would you like to use your camera to take a photo?\nPress 'OK' for Camera or 'Cancel' to upload from gallery."
//     //   );
//     //   setCaptureMode(useCamera ? "camera" : "upload");
//     // }
...
//                 {isMobile ? "Tap to snap or upload" : "Click to upload"}
//               </span>
//             </p>
...
export function IngredientRecognitionForm({
  ...
  const handleClick = () => {
    // Show the upload/camera choice popup
    setShowOptions(true);
  };

  const handleTakePhoto = () => {
    // Opens camera first (works on both desktop and mobile)
      ...
      >
        ...
        {photoDataUri ? (
          ...
          <div className="text-center">
            <Camera className="mx-auto h-12 w-12 text-muted-foreground" />
            <p className="mt-2 text-sm text-muted-foreground">
              <span className="font-semibold text-primary">Snap a Photo</span>{" "}
              or upload an image
            </p>
            <p className="text-xs text-muted-foreground">PNG, JPG Only</p>
          </div>
        )}
      </div>

      {/* Camera/Upload choice popup */}
      {showOptions && (
        <div className="flex justify-center gap-4 mt-2">
          <Button type="button" variant="secondary" onClick={handleTakePhoto}>
